      ...
      return parsedComments.map((comment, index) => {
        ...
        const commentAvatar =
          userInfo && userInfo[comment.userId]
            ? userInfo[comment.userId].avatar || ''
            : '';
        ...
        return (
          <ListItem
            key={index}
            style={{ height: '60px', position: '', zIndex: '' }}
            innerDivStyle={{ padding: '6px 16px 16px 72px' }}
            leftAvatar={
              <UserAvatar
      ...
      <div>
        ...
        >
          ...
          <Paper
            ...
            style={
              !this.props.open ? { display: 'block' } : { display: 'none' }
            }
          >
            ...
            >
              <div style={{ display: 'flex' }}>
                <UserAvatar
                  ...
                  style={{ flex: 'none', margin: '4px 0px' }}
                  size={36}
                />
                <div
                  style={{
...
const mapStateToProps = (state, ownProps) => ({
  ...
  avatar:
    state.user.info && state.user.info[state.authorize.uid]
      ? state.user.info[state.authorize.uid].avatar || ''
      : '',
  fullName:
    state.user.info && state.user.info[state.authorize.uid]
      ? state.user.info[state.authorize.uid].fullName || ''
      : '',
